...
function empilha() {
  ...
  if ($(".td-acao").last().text().split("> ")[1] === "&") {
    ...
    $(".tr-sintatico")
      .last()
      .append(tdPilha(texto_pilha.substr(0, texto_pilha.length - 1) + ultima_seq));
    $(".tr-sintatico")
      .last()
      .append(tdEntrada($(".td-entrada").last().text()));
  }
...
function cedulaComparavel(linha, coluna) {
  ...
  if (linha === "k" && coluna === "s") {
    ...
    return $(".tr-sintatico")
      .last()
      .append(tdAcao("OK em " + $(".tbody-sintatico").find("tr").length + " iterações"));
    ...
    if (cedula != "") {
      ...
      return $(".tr-sintatico")
        .last()
        .append(tdAcao("Erro em " + $(".tbody-sintatico").find("tr").length + " iterações"));
    }
  }
}
